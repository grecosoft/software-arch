...
echo "-------------------------------------------------------"
echo "RUN:    kubectl port-forward \"service/seq\" -n examples 8081:80"
echo "OPEN:   http://localhost:8081"

rabbit_username="$(kubectl get secret microservice-integration-default-user -n examples -o jsonpath='{.data.username}' | base64 --decode)"
rabbit_password="$(kubectl get secret microservice-integration-default-user -n examples -o jsonpath='{.data.password}' | base64 --decode)"

echo ""
echo "-------------------------------------------------------"
echo "COMPLETE THE FOLLOWING TO VIEW THE RABBITMQ DASHBOARD"
echo "-------------------------------------------------------"
echo "RUN:    kubectl port-forward \"service/microservice-integration\" -n examples 15672"
